* ============================================================================
* t14_basics — the oracle-checked basics: use / save / merge / append, two ways
*
* Formerly examples/parqit_basics.do; the public crash course of that name now
* ships with the SSC package and carries no oracle. This test walks the four
* base operations twice over the same small data:
*
*   A. the eager way: read everything into Stata's memory first, then work —
*      the right mental model when the data is small; and
...
*
* Every lazy result is asserted against a native-Stata twin; the test ends in
* VERDICT(PARQIT_BASICS): PASS. Runtime: seconds.
*
* Usage (tests/run_stata.sh supplies the arguments):
*   . do t14_basics.do <repo_root> <plugin>     (development tree)
*   . do t14_basics.do                          (parqit already on the adopath)
* ============================================================================
clear all
set more off
set linesize 100
...
if (`"`repo'"' != "") {
    ...
    * no-argument mode: if parqit is not on the adopath yet, try the repo-local
    * install tree (this file lives in <repo>/tests/integration, the tree in
    * <repo>/ado/plus/p)
