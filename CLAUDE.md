...
Current state: **v0.1.19** (see [CMakeLists.txt](CMakeLists.txt) `project(... VERSION)`). The full command surface in `README.md` is implemented (single-table verbs, two-table verbs, reshape, pivot, sql/query, native `mergein`/`appendin`, dialogs, metadata round-trip). Milestones M0–M4 (brief §11) are in the tree; M5 is SSC-release polish. The version/date lives in **four** synchronised surfaces — `CMakeLists.txt`, the `parqit.ado` banner, the `parqit.sthlp` banner, and `README.md` **Status** + `parqit.pkg` `Distribution-Date` — and `tests/release_lint.sh` fails the build if they drift. Bump them together.
