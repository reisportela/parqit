...
program define profile_parqit, rclass
    version 16.0
    gettoken todo 0 : 0, parse(" ,")
    if `"`todo'"' == "" {
        di as err "profile_parqit: subcommand required; see {help parqit}"
        exit 198
    }
    ...
    local cmds version selftest use save describe glimpse open close       ///
        ...
        tabstat correlate pwcorr histogram
...
program define _parqit_use, rclass
    ...
    * parqit open _data passes it for its per-promotion bridge snapshots.
    capture syntax [namelist] using/ [, clear Name(name) OWNed]
    if (_rc) {
        syntax anything(name=fileraw id="filename") [, clear Name(name) OWNed]
        local using `fileraw'
        local namelist
    }
    _parqit_ensure_plugin
...
program define _parqit_egen
    ...
    * parqit egen [type] name = fcn(expr) [, by(varlist)]
    ...
    confirm name `name'
    gettoken fcall 0 : 0, parse(",")
    local fcall = strtrim(`"`fcall'"')
    local p = strpos(`"`fcall'"', "(")
    if (`p' == 0 | substr(`"`fcall'"', -1, 1) != ")") {
        di as err "parqit egen: expected fcn(expression)"
        exit 198
    }
    local fcn = substr(`"`fcall'"', 1, `p' - 1)
    local fexpr = substr(`"`fcall'"', `p' + 1, strlen(`"`fcall'"') - `p' - 1)
    syntax [, by(string)]
    _parqit_ensure_plugin
    tempfile req
    local _sq_name "`name'"
...
program define _parqit_rename
    ...
    if (`"`oldn'"' == "" | `"`newn'"' == "" | strtrim(`"`0'"') != "") {
        di as err "parqit rename: syntax is parqit rename oldname newname"
        exit 198
    }
    confirm name `newn'
    _parqit_ensure_plugin
...
program define _parqit_collect, rclass
    ...
    di as txt "view_collect_prepare took " as res %8.3f r(t10) " sec"
    ...
    * parqit merge 1:1|m:1|1:m|m:m keys using <file> [, keep() keepusing() gen() nogen]
    ...
    if ("`keep'" != "") {
        foreach w of local keep {
            if inlist("`w'", "master", "1") local mask = `mask' + 1
            else if inlist("`w'", "using", "2") local mask = `mask' + 2
            else if inlist("`w'", "match", "matched", "3") local mask = `mask' + 4
            else {
...
{
    string scalar    j, name, vlname, ent
...
void _parqit_apply_strl(string scalar path)
{
    ...
    fh = fopen(path, "r")
    while ((hdr = fread(fh, 32)) != J(0, 0, "")) {
        if (strlen(hdr) < 32) break
        v   = strtoreal(substr(hdr, 1, 10))
        o   = strtoreal(substr(hdr, 11, 10))
        len = strtoreal(substr(hdr, 21, 12))
...
{
    real scalar      fh, nv, i, v
    string scalar    line, name, varlab, vallab, labname, txt, tgt, cname
    string rowvector f
    string colvector vl_names, vl_texts_all, vl_owner
    ...
    while ((line = fget(fh)) != J(0, 0, "")) {
        ...
        if (f[1] == "var") {
            ...
            if (vallab != "" & st_isnumvar(name)) st_varvaluelabel(name, vallab)
